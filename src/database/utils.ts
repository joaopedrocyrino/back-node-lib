export const whereBuilder = (where: { [k: string]: any } | Array<{ [k: string]: any }>): string => {
    let query = ''

    if (Array.isArray(where)) {
        where.forEach(w => { query += `(${whereBuilder(w)}) OR ` })
...
export const commaBuilder = (fields: string[]): string => {
    let query = ''
...
export const setBuilder = (fields: { [k: string]: string | number | boolean }): string => {
    let query = ''
